    ...
    <>
      <header className="fixed top-0 left-0 w-full z-[100] px-6 md:px-12 py-6 md:py-8 flex justify-between items-center pointer-events-none">
        ...
        <div className="flex items-center gap-4 md:gap-8 pointer-events-auto">
          <nav className="hidden md:flex items-center gap-4 lg:gap-8 mr-4">
            {navItems.map((item) => (
              <a
                key={item}
                href={`#${item.toLowerCase()}`}
                className={`text-[9px] lg:text-[10px] uppercase tracking-[0.2em] lg:tracking-[0.3em] opacity-40 hover:opacity-100 transition-opacity ${
                  theme === "dark" ? "text-white" : "text-black"
                }`}
              >
                {item}
          ...
          <a 
            ...
            className={`hidden md:block px-6 py-3 rounded-full text-[9px] font-bold uppercase tracking-[0.2em] border transition-all duration-500 hover:scale-105 active:scale-95 ${
              theme === "dark" 
          ...
          <div className={`w-8 h-8 md:w-9 md:h-9 rounded-full border overflow-hidden shadow-lg shadow-black/50 ${
